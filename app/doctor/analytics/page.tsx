...
export default function AnalyticsPage() {
  ...
  const StatCard = ({ title, value, change, icon: Icon, color, prefix = "", suffix = "" }) => (
    <Card className="analytics-stat-card relative overflow-hidden group hover:shadow-2xl transition-all duration-500 hover:-translate-y-1 border border-slate-200 bg-white backdrop-blur-sm">
      <div
      ...
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <CardTitle className="text-xs sm:text-sm font-medium text-slate-700">{title}</CardTitle>
        <div
      ...
      <CardContent>
        <div className="text-xl sm:text-2xl lg:text-3xl font-bold text-slate-900 mb-1 transition-all duration-300 group-hover:scale-110">
          {isLoading ? (
            <div className="h-6 sm:h-8 w-12 sm:w-16 bg-slate-200 rounded animate-pulse" />
          ) : (
            `${prefix}${value}${suffix}`
          )}
        </div>
  ...
  const ChartCard = ({ title, description, children }) => (
    <Card className="analytics-chart-card border border-slate-200 bg-white backdrop-blur-sm">
      <CardHeader>
  ...
  const SimpleBarChart = ({ data, dataKey, color = "teal" }) => (
    <div className="space-y-3 bar-chart-container">
      {data.map((item, index) => (
        <div key={index} className="flex items-center space-x-3">
          <span className="text-slate-600 text-sm w-8">{item.month}</span>
          ...
          <span className="text-slate-600 text-sm w-8 text-right">{item[dataKey]}</span>
        </div>
      ))}
    </div>
  )
      ...
      <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-6">
        <div className="relative w-32 h-32 sm:w-40 sm:h-40">
          <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
            <circle cx="50" cy="50" r="40" fill="none" stroke="rgb(226 232 240)" strokeWidth="8" />
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-slate-900 font-bold text-lg sm:text-xl">{total}</span>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8 py-4 sm:py-8">
          ...
          <div className="analytics-header flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 sm:mb-8 space-y-4 sm:space-y-0">
            ...
            <Button
              ...
              size="sm"
              onClick={() => router.back()}
              className="mr-3 sm:mr-4 border-slate-300 text-slate-700 hover:bg-slate-100 hover:text-slate-900 bg-white"
            >
              <ArrowLeft className="w-4 h-4 mr-1 sm:mr-2 text-slate-700 group-hover:text-slate-900" />
              <span className="hidden sm:inline">Back</span>
            </Button>
            <div>
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold bg-gradient-to-r from-slate-900 via-slate-700 to-slate-800 bg-clip-text text-transparent">
                Analytics Dashboard
              </h1>
              <p className="text-slate-500 text-sm sm:text-lg mt-1 sm:mt-2">Track your practice performance</p>
            </div>
          </div>
          <Select value={timeRange} onValueChange={setTimeRange}>
            <SelectTrigger className="analytics-time-range w-full sm:w-48">
              <SelectValue placeholder="Select time range" />
            </SelectTrigger>
            <SelectContent className="analytics-time-range-content">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
            ...
            <ChartCard title="Quick Stats" description="Key performance indicators">
              <div className="space-y-4">
                <div className="flex items-center justify-between p-3 bg-slate-100 rounded-lg quick-stat-item">
                  <div className="flex items-center space-x-2">
                    <Clock className="w-4 h-4 text-teal-600" />
                    <span className="text-slate-600 text-sm">Avg Session</span>
                  </div>
                  <span className="text-slate-900 font-semibold">{analytics.avgSessionTime} min</span>
                </div>
                <div className="flex items-center justify-between p-3 bg-slate-100 rounded-lg quick-stat-item">
                  <div className="flex items-center space-x-2">
                  ...
                  <span className="text-slate-900 font-semibold">{analytics.completedAppointments}</span>
                </div>
                <div className="flex items-center justify-between p-3 bg-slate-100 rounded-lg quick-stat-item">
                  <div className="flex items-center space-x-2">
                    <Users className="w-4 h-4 text-blue-600" />
                    <span className="text-slate-600 text-sm">New Patients</span>
                  </div>
                  <span className="text-slate-900 font-semibold">{Math.floor(analytics.totalPatients * 0.3)}</span>
                </div>
                <div className="flex items-center justify-between p-3 bg-slate-100 rounded-lg quick-stat-item">
                  <div className="flex items-center space-x-2">
