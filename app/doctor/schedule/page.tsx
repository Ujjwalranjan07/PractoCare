...
export default function SchedulePage() {
  ...
  const DayScheduleCard = ({ day, dayData }) => {
    ...
    return (
      <Card className="border border-slate-200 bg-white backdrop-blur-sm hover:shadow-xl transition-all duration-300 day-schedule-card">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0">
            <div className="flex items-center space-x-3">
              <Switch
                ...
                className="data-[state=checked]:bg-blue-500 toggle-switch"
              />
              <Label className="text-slate-900 font-medium text-sm sm:text-base">{day.label}</Label>
            </div>
            <div className="flex items-center space-x-2">
              <Badge
                className={
                  dayData.enabled
                    ? "bg-green-500/20 text-green-300 border-green-500/30 status-badge active"
                    : "bg-red-500/20 text-red-300 border-red-500/30 status-badge inactive"
                }
              >
                ...
                <Dialog>
                  <DialogTrigger asChild>
                    <Button
                      size="sm"
                      variant="outline"
                      className="border-slate-300 text-slate-700 bg-white hover:bg-slate-100 schedule-btn"
                      onClick={() => setEditingDay(day.key)}
                    >
                      <Edit className="w-3 h-3 sm:w-4 sm:h-4" />
                    </Button>
                  </DialogTrigger>
                  <DialogContent className="bg-white border-slate-200 max-w-md enhanced-dialog enhanced-dialog-content">
                    <DialogHeader>
                      <DialogTitle className="text-slate-900">Edit {day.label} Schedule</DialogTitle>
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <Label className="text-slate-600 text-sm">Start Time</Label>
                          <Input
                            type="time"
                        ...
                        <div>
                          <Label className="text-slate-600 text-sm">End Time</Label>
                          <Input
                            type="time"
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <Label className="text-slate-600 text-sm">Break Start</Label>
                          <Input
                            type="time"
                        ...
                        <div>
                          <Label className="text-slate-600 text-sm">Break End</Label>
                          <Input
                            type="time"
                      ...
                      <div>
                        <Label className="text-slate-600 text-sm">Slot Duration (minutes)</Label>
                        <Select
        ...
        <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8 py-4 sm:py-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 sm:mb-8 space-y-4 sm:space-y-0 schedule-header">
            ...
            <Button
              ...
              size="sm"
              onClick={() => router.back()}
              className="mr-3 sm:mr-4 border-slate-300 text-slate-700 hover:bg-slate-100 hover:text-slate-900 bg-white group schedule-btn"
            >
              <ArrowLeft className="w-4 h-4 mr-1 sm:mr-2 text-slate-700 group-hover:text-slate-900" />
              <span className="hidden sm:inline">Back</span>
            </Button>
            <Button
              variant="outline"
              size="sm"
              onClick={() => router.push("/doctor/appointments?view=calendar")}
              className="mr-3 sm:mr-4 border-blue-300 text-blue-700 hover:bg-blue-100 hover:text-blue-900 bg-white group schedule-btn"
            >
              <Calendar className="w-4 h-4 mr-1 sm:mr-2 text-blue-700 group-hover:text-blue-900" />
              <span className="hidden sm:inline">View Calendar</span>
            </Button>
            <div>
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold bg-gradient-to-r from-slate-900 via-slate-700 to-slate-800 bg-clip-text text-transparent animate-pulse-glow">
                Schedule Management
              </h1>
              <p className="text-slate-500 text-sm sm:text-lg mt-1 sm:mt-2">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-4 gap-6 sm:gap-8">
            {/* Weekly Schedule */}
            <div className="xl:col-span-3 schedule-section">
              <Card className="border border-slate-200 bg-white backdrop-blur-sm shadow-lg mb-6 sm:mb-8">
                <CardHeader>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                {days.map((day, index) => (
                  <div key={day.key} style={{ animationDelay: `${index * 100}ms` }} className="animate-fadeInUp">
            ...
            {/* Sidebar */}
            <div className="space-y-6 schedule-section">
              {/* Quick Stats */}
              <Card className="border border-slate-200 bg-white backdrop-blur-sm shadow-lg stats-card">
                <CardHeader>
                  <CardTitle className="text-lg sm:text-xl font-bold text-slate-900">Quick Stats</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div className="flex justify-between items-center">
                    <span className="text-slate-600 text-sm">Active Days</span>
                    <Badge className="bg-green-100 text-green-700 border-green-300">
                      {Object.values(schedule).filter((day) => day.enabled).length}/7
                    </Badge>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-slate-600 text-sm">Total Slots</span>
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-slate-600 text-sm">Blocked Dates</span>
                    <Badge className="bg-red-100 text-red-700 border-red-300">{blockedDates.length}</Badge>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="border border-slate-200 bg-white backdrop-blur-sm shadow-lg stats-card">
                <CardHeader>
                  <CardTitle className="text-lg sm:text-xl font-bold text-slate-900">Block Dates</CardTitle>
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Button
                      ...
                      className="w-full bg-red-500 hover:bg-red-600 text-white text-sm schedule-btn"
                    >
                  ...
                  {blockedDates.length > 0 && (
                    <div className="space-y-2 max-h-48 overflow-y-auto">
                      {blockedDates.map((block) => (
                        <div
                            key={block.id}
                            className="flex items-center justify-between p-2 bg-slate-100 rounded text-sm blocked-date-item"
                          >
                          <div>
                            <div className="text-slate-900 font-medium">{new Date(block.date).toLocaleDateString()}</div>
                          ...
                          <Button
                            ...
                            className="border-red-300 text-red-700 hover:bg-red-100 schedule-btn"
                          >
